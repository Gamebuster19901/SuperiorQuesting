...
public class PacketPageLock extends GenericQuestingPacket{
	public UUID id;
	public List<UUID> players = new ArrayList<UUID>();
	
	public PacketPageLock() {
		super(PacketType.PAGE_LOCK);
	}
	
	public PacketPageLock(Page p) {
		this();
		id = p.getUUID();
	}
	
	public PacketPageLock(Page p, UUID... players) {
		this();
		id = p.getUUID();
